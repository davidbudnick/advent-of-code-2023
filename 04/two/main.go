...
func main() {
	...
	for scanner.Scan() {
		...
		games[gameID] = len(yourWinningNumbers)

		gameID++
	}

	cards := make(map[int]int)
	for i := 1; i < len(games)+1; i++ {
		cards[i] = 1
	}

	for i := 1; i < len(games)+1; i++ { //first loop over the cards
		for k := 0; k < cards[i]; k++ { //loops over if there are more copies
			for j := 0; j < games[i]; j++ { //process the og and copies
				cards[i+j+1]++
			}
		}
	}
	...
	slog.Info("Total Cards", "CARDS", totalCards) //5539496
}
